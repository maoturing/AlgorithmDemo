...
/**
 * 暴风(Brute Force)算法是普通的模式匹配算法，BF算法的思想就是将目标串Original的第一个字符与模式串find的第一个字符进行匹配，<br/>
 * 若相等，则继续比较O的第二个字符和 f的第二个字符；若不相等，则比较O的第二个字符和f的第一个字符，依次比较下去，直到得出最后的匹配结果。<br/>
 * BF算法是一种蛮力算法。<br/>
 * 给定两个字符串O和f，长度分别为n和m，判断f是否在O中出现，<b>复杂度是O(nm)</b><br/>
